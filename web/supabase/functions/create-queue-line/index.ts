...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const serviceAreaId = typeof input.service_area_id === 'string' && input.service_area_id
      ? input.service_area_id
      : null
    let serviceArea: { id: string, name: string } | null = null
    if (serviceAreaId) {
      const { data, error: serviceAreaError } = await adminClient
        .from('service_areas')
        .select('id, name')
        .eq('id', serviceAreaId)
      ...
      if (!data) return json({ error: 'The selected service area is no longer available.' }, 400)
      serviceArea = data
    }
