...
- **v2（真實錄影增強）**: val 91.2% / test 90.6% — 對訓練同款條件（靜止段+變速）93–98% 穩健
- v3（+i.i.d. 抖動+翻轉增強）: val 91.1% / test 89.5% — 模擬評測與 v2 打平（31% vs 34%）
- **結論：合成增強已到天花板。** 模擬噪聲（逐幀獨立、經肩點歸一化放大）與真實
  MediaPipe 時間相關抖動存在本質差異，小動作詞（mana/berapa）在模擬條件下無法可靠辨識。
  **下一步必須用真人小樣本微調**（finetune_custom.py，每詞 8–30 段即可）。
