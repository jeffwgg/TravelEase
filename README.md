...
The backend is a hosted [Supabase](https://supabase.com) project (Postgres, Auth, Realtime, Edge Functions). Translation can use a self-hosted LibreTranslate instance run with Docker.
...
Secrets used only by Supabase Edge Functions (`LIBRETRANSLATE_URL`, `LIBRETRANSLATE_API_KEY`, `TRANSLATION_PROVIDER`, `RESEND_API_KEY`) are stored with `supabase secrets set`, not in any `.env` file. See the LibreTranslate note below when self-hosting translation.
...
supabase secrets set TRANSLATION_PROVIDER=libretranslate
supabase secrets set RESEND_API_KEY=<your-resend-key>
```

## Docker: self-hosted translation service (LibreTranslate)

Announcement and mobile-text translation can use self-hosted LibreTranslate.
The TravelEase Docker service is **not online at all times**; contact the
TravelEase team when you need the existing service to be started.

To run an independent instance, you need Docker, a public HTTPS URL that
Supabase can reach, and access to configure the associated Supabase project's
`LIBRETRANSLATE_URL`, `LIBRETRANSLATE_API_KEY`, and (for announcements)
`TRANSLATION_PROVIDER=libretranslate` secrets. Do not change translation
secrets in a shared project without the team's approval.

For the Docker commands and Supabase connection steps, see
[web/supabase/libretranslate/README.md](web/supabase/libretranslate/README.md).
