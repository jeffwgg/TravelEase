...
class _SignTranslationCameraViewState extends State<SignTranslationCameraView>
  ...
  final _speechTourTargetKey = GlobalKey();
  ...
  void initState() {
    ...
    FeatureUsageTracker.instance.opened(
      widget.initialMode == SignTranslationMode.signToText
          ? TrackedFeature.signTranslate
          : TrackedFeature.speechToSign,
    );
    ...
    try {
      ...
      try {
        _cameraController!.startImageStream((CameraImage image) async {
          if (!_cameraViewModel.isDetecting ||
              _currentMode != SignTranslationMode.signToText)
            return;
  ...
  void _trackSpeechToSignCompletion() {
    if (_hasRecordedSpeechToSign || !_speechViewModel.hasContent) return;
    _hasRecordedSpeechToSign = true;
    FeatureUsageTracker.instance.completed(TrackedFeature.speechToSign);
  }
          ...
          children: [
                    ...
                    Padding(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<SignLanguageType>(
                          ...
                          onChanged: (v) {
                            if (v != null)
                              _speechViewModel.switchSignDialect(v);
                          },
                        ),
                      ),
                    ),
            ...
            Positioned.fill(
              child: AppTourCoachmark(
                feature: widget.initialMode == SignTranslationMode.signToText
                    ? AppTourFeature.signTranslate
                    : AppTourFeature.speechToSign,
                targetKey: isSignToText
                    ? _signTourTargetKey
                    : _speechTourTargetKey,
                title: isSignToText ? 'Sign Translate' : 'Speech to Sign',
                message: isSignToText
                    ? 'Show your sign to the camera.'
                    : 'Speak or type to see the sign.',
              ),
            ),
          ],
        );
