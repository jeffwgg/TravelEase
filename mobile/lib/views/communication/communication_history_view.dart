...
import '../../viewmodels/communication_history_viewmodel.dart';

/// Saved conversation browser (FR-M3-17, FR-M3-18, UC304).
...
class CommunicationHistoryView extends StatefulWidget {
  const CommunicationHistoryView({super.key});
...
class _CommunicationHistoryViewState extends State<CommunicationHistoryView> {
  late final CommunicationHistoryViewModel _viewModel;
  String _filter = 'all';
  ...
  void initState() {
    super.initState();
    _viewModel = CommunicationHistoryViewModel()..loadSavedLogs();
  }
  ...
  Future<void> _confirmDelete(ConversationLog log) async {
    ...
    if (ok == true) await _viewModel.deleteSavedLog(log.id);
  }
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        final logs = _viewModel.savedLogs;
        ...
        return Scaffold(
          ...
          body: Column(
            children: [
              ...
              // ── Log list ──
              Expanded(
                child: _viewModel.isLoadingLogs
                    ? const Center(child: CircularProgressIndicator())
                    : filtered.isEmpty
                    ? _buildEmptyState()
                    : RefreshIndicator(
                        onRefresh: _viewModel.loadSavedLogs,
  ...
  Widget _buildLogCard(ConversationLog log) {
    ...
    final typeIcon =
        _typeIcons[log.translationType] ?? Icons.description_rounded;
